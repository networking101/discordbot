        ...
        if (Object.keys(addRuleJSON).length > 3){
            if(addRuleJSON["event"]){
                if (addRuleJSON["flags"] & 0x80000000){
                    addCondition(client, message, addRuleJSON)
                }
                else{
                    if (addRuleJSON["event"] === "message"){
                        addMessage(client, message, addRuleJSON)
                    }
                    else if (addRuleJSON["event"] === "voice") {
                        addVoice(client, message, addRuleJSON)
                    }
                }
            }
            else{
...
function addMessage(client, message, addRuleJSON) {
...
function addVoice(client, message, addRuleJSON) {
    let completeFlag = addRuleJSON["flags"]
    let currentKey = addRuleJSON[Object.keys(addRuleJSON)[5]]
    if(completeFlag & 0x1){
        if(!(completeFlag & 0x2)){
...
function addCondition(client, message, addRuleJSON) {
    let completeFlag = addRuleJSON["flags"]
    let currentKey = addRuleJSON[Object.keys(addRuleJSON)[5]]
    if(completeFlag & 0x1){
        if(completeFlag & 0x2){
